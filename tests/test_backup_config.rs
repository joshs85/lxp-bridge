...
fn create_test_config() -> ConfigWrapper {
    ConfigWrapper::new("tests/fixtures/config.yaml".to_string()).unwrap()
}
...
fn test_config_loading_logic() {
    ...
    let config_result = ConfigWrapper::new("tests/fixtures/config.yaml".to_string());
...
fn test_config_error_handling() {
    ...
    let config_result = ConfigWrapper::new("nonexistent_file.yaml".to_string());
...
fn test_backup_command_creation() {
    ...
    let _backup_cmd = BackupConfigCommand::new(inverter.clone(), output_path.to_string());
...
fn test_env_logger_integration() {
    ...
    let _builder = env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("info"));
...
fn test_error_propagation() {
    ...
    let config_result: Result<ConfigWrapper, anyhow::Error> = 
        ConfigWrapper::new("nonexistent_file.yaml".to_string());
