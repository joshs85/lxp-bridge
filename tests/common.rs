...
impl Factory {
    ...
    pub fn config_with_individual_input_enabled() -> config::ConfigWrapper {
        let mut config = Self::example_config();
        config.mqtt.publish_individual_input = Some(true);
        ConfigWrapper::new_from_config(config)
    }
